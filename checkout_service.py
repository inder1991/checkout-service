...
# Thread-safe locking mechanism for in-memory inventory
inventory_locks = {}
lock_manager = threading.Lock()

async def check_and_decrement_stock(item_id: str, quantity: int, trace_id: str = None):
    """
    FIXED: Race condition resolved with thread-safe locking.
    
    The check and decrement are now atomic operations protected by a lock.
    This prevents multiple concurrent requests from causing negative stock.
    """
    log_json("INFO", "Decrementing inventory stock",
            trace_id=trace_id, 
            item_id=item_id,
            requested_quantity=quantity,
            locking_mechanism="thread_lock",
            atomic_operation=True)
    
    try:
        # Ensure lock exists for this item (thread-safe initialization)
        with lock_manager:
            if item_id not in inventory_locks:
                inventory_locks[item_id] = threading.Lock()
        
        # Acquire item-specific lock for atomic check-and-decrement
        with inventory_locks[item_id]:
            current_stock = inventory_stock.get(item_id, 0)
            
            # RESTORED: Business logic validation
            # Check if sufficient stock is available
            if current_stock < quantity:
                error_msg = f"Insufficient stock: item_id={item_id} available={current_stock} requested={quantity} lock_type=thread_lock atomic=true"
                raise RuntimeError(error_msg)
            
            # Artificial delay to simulate real-world processing time
            # With proper locking, this no longer causes race conditions
            await asyncio.sleep(0.1)
            
            # Atomic decrement - protected by lock
            new_stock = current_stock - quantity
            inventory_stock[item_id] = new_stock
            
            # Verify stock is non-negative (defensive check)
            if inventory_stock[item_id] < 0:
                # This should never happen with proper locking, but log if it does
                error_msg = f"CRITICAL: Negative stock detected despite locking: item_id={item_id} stock_level={inventory_stock[item_id]} lock_type=thread_lock atomic=true"
                raise RuntimeError(error_msg)
        ...
        log_exception(
            ...
            exc=e,
            trace_id=trace_id,
            span_id=f"stock-decrement-{int(time.time())}",
            error_code="STOCK_DECREMENT_ERROR",
            item_id=item_id,
            requested_quantity=quantity,
            current_stock=inventory_stock.get(item_id, 0),
            locking_mechanism="thread_lock",
...
async def checkout(request: CheckoutRequest):
    ...
    try:
        ...
        for item in request.items:
            ...
            # FIXED: Internal race condition check with proper locking
            try:
                await check_and_decrement_stock(item.item_id, item.quantity, trace_id)
            except RuntimeError as e:
                # This can now be either insufficient stock OR negative stock
                error_msg = str(e)
                if "Insufficient stock" in error_msg:
                    # Business logic error - insufficient stock available
                    log_json("WARN", "Insufficient stock available",
                            trace_id=trace_id, 
                            item_id=item.item_id,
                            severity="medium")
                    raise HTTPException(status_code=409, detail="Insufficient stock available")
                else:
                    # Technical bug - negative stock (should not happen with fix)
                    log_exception("ERROR", "Critical: Negative stock detected despite locking",
                            exc=e,
                            trace_id=trace_id, severity="critical")
                    raise HTTPException(status_code=500, detail="Stock management error")
        ...
        try:
            ...
            payment_data = {"transaction_id": internal_txn.get("transaction_id")}
