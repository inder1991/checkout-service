...
def get_item_lock(item_id: str) -> threading.Lock:
    ...
    Returns:
        threading.Lock: Lock object for the specified item
    """
    with lock_manager_lock:
        if item_id not in inventory_locks:
            inventory_locks[item_id] = threading.Lock()
        return inventory_locks[item_id]
...
async def check_and_decrement_stock(item_id: str, quantity: int, trace_id: str = None):
    ...
    This function now uses pessimistic locking to prevent race conditions
    where multiple concurrent requests could cause negative stock levels.
    ...
    try:
        # Acquire lock - ensures only one thread processes this item at a time
        with item_lock:
            ...
            # FIXED: Atomic check-then-act with lock protection
            # The lock ensures no other thread can modify stock between check and update
            new_stock = current_stock - quantity
            inventory_stock[item_id] = new_stock
            
            # Validation check - should never trigger with proper locking
            if inventory_stock[item_id] < 0:
                error_msg = f"Negative stock detected: item_id={item_id} stock_level={inventory_stock[item_id]} lock_type=pessimistic atomic=true"
        ...
        # This should not happen with proper locking, but log if it does
        log_exception(
            "ERROR",
            ...
            locking_mechanism="pessimistic",
            atomic_operation=True,
            concurrent_requests_likely=False,
            severity="critical",
    ...
    try:
        ...
        for item in request.items:
            ...
            try:
                ...
                log_exception("ERROR", "Race condition detected despite fix",
                        exc=e,
        ...
        try:
            ...
            log_json("WARN", "Credit validation error (continuing anyway)",
                    trace_id=trace_id)
            pass  # Continue - business logic errors ignored
