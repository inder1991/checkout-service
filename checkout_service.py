...
async def check_and_decrement_stock(item_id: str, quantity: int, trace_id: str = None):
    ...
    with item_lock:
        try:
            ...
            # CRITICAL: Validate stock availability BEFORE decrementing
            ...
            # Additional safety check (should never trigger with proper validation above)
            if new_stock < 0:
            ...
            log_exception(
                ...
                error_code="STOCK_OPERATION_ERROR",
                item_id=item_id,
                stock_level=inventory_stock.get(item_id, 0),
                locking_mechanism="threading.Lock",
                atomic_operation=True,
                severity="high",
